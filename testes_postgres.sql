...
DECLARE
    ...
    test_airport_iata VARCHAR(3) := 'ZZZ';
BEGIN
    ...
    INSERT INTO CompanhiaAerea(nome)
    VALUES ('Test Airline');
    ...
    -- TESTE 12: FOREIGN KEY id
    BEGIN
        INSERT INTO Comentario(conteudo, usuario_username, companhia_id) 
        VALUES ('Teste Conteudo', test_user, 999999);
        PERFORM log_test('FK id companhia', FALSE, 'Inseriu com id de companhia inválido');
    EXCEPTION WHEN foreign_key_violation THEN
        PERFORM log_test('FK id companhia', TRUE);
    END;
    ...
    BEGIN
        INSERT INTO Comentario(conteudo, usuario_username, aeroporto_iata, companhia_id) 
        VALUES ('Teste Conteudo', test_user, test_airport_iata, 1254);
    ...
    BEGIN
        UPDATE Aeroporto SET iata = 'QQQ' WHERE iata = test_airport_iata;
        
        IF EXISTS (SELECT 1 FROM Comentario WHERE aeroporto_iata = 'QQQ') THEN
	...
	DELETE FROM Comentario WHERE usuario_username = test_user OR aeroporto_iata IN (test_airport_iata, 'QQQ');
    DELETE FROM Usuario WHERE username in (test_user, test_admin);
    DELETE FROM Aeroporto WHERE iata IN (test_airport_iata, 'QQQ');
    DELETE FROM CompanhiaAerea WHERE id = 1254;
END;
$$ LANGUAGE plpgsql;
